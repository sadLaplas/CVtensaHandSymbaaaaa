...
class ConfigTests(unittest.TestCase):
    ...
    def test_load_config_successfully(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(config.dataset.gesture_labels, ("open_palm", "fist"))
            self.assertEqual(
                config.mediapipe.model_path,
                (root / "models" / "hand_landmarker.task").resolve(),
            )
            self.assertEqual(
                config.dataset.index_file,
                (root / "data" / "raw" / "dataset_index.csv").resolve(),
            )

    def test_load_config_without_dataset_section_uses_defaults(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            config_dir = root / "configs"
            config_dir.mkdir(parents=True, exist_ok=True)
            ...
            self.assertEqual(config.dataset.capture_interval_ms, 250)
            self.assertEqual(config.dataset.max_samples_per_label, 250)
            self.assertEqual(config.dataset.gesture_labels[0], "open_palm")
